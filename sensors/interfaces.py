...
class GPIOProcess(mp.Process):
    ...
    """
    def __init__(self, address, gpio_queue, *args, **kwargs):
    ...
    def check_for_new_value(self):
        ...
        """
        # new_value = self._gpio.input(self.address)
        # if new_value != self.last_record.value:
        if not self._gpio_queue.empty():
            new_value = self._gpio_queue.get()
            print("checked value: {}".format(new_value))
